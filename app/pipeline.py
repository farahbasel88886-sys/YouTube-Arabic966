...
def run_pipeline(
    ...
    # ------------------------------------------------------------------
    notify(2, "Normalizing audio")
    logger.info("[bold]Step 2/5 — Normalizing audio[/bold]")
    normalized_path = audio.normalize_audio(raw_audio_path, temp_dir)

    # ------------------------------------------------------------------
    # Step 3: Transcribe
    # ------------------------------------------------------------------
    notify(3, "Transcribing with Whisper (local)")
    logger.info("[bold]Step 3/5 — Transcribing[/bold]")
    selected_mode = transcriber.normalize_transcription_mode(
        transcription_mode or settings.TRANSCRIPTION_MODE
    )
    ...
    # Transcript-first flow: persist a display-ready transcript immediately.
    save_text(output_dir / "transcript_ar.md", transcription.raw_text)
    logger.info("Saved: transcript_ar.md")

    # ------------------------------------------------------------------
    # Step 4: Save metadata only (generation is triggered separately)
    # ------------------------------------------------------------------
    notify(4, "Saving transcript and metadata")
